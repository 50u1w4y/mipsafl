...
static const u8* trampoline_fmt_32 =
  ...
  /* 注意我们插桩的位置
     条件分支紧随其后，条件分支跳转的块在一开始的位置
     函数开头是插在 .frame 等一开始的伪指令后面
     addiu 前面的
     也就是说，$gp 已经被初始化好了 */

  /* 只需要对我们会操作到的寄存器进行保存就行了
     不用保存原来的 $sp, 因为回来时 $sp 的位置是不变的 */
...
/* 两个重要功能:
   1. 生成 fork server 并让 server fork 子进程执行测试用例
   2. 进行覆盖率报告(通过共享内存进行报告) */
...
static const u8* main_payload_32 =
  ...
  "  .align 2\n"
  "$LCMY:\n"
  "  .ascii \"__AFL_SHM_ID\\000\"\n"
  "  .section	.data.rel.local,\"aw\",@progbits\n"
  "  .align 2\n"
  "  .type AFL_SHM_ENV, @object\n"
  "  .size AFL_SHM_ENV, 4\n"
  "AFL_SHM_ENV:\n"
  "  .word $LCMY\n"
  "  .rdata\n"
  "  .align 2\n"
  "$LCSTR1:\n"
  "  .ascii \"This is test\\000\"\n"
  "  .align 2\n"
  "$LCSTR2:\n"
  "  .ascii \"w\\000\"\n"
  "  .align 2\n"
  "$LCSTR3:\n"
  "  .ascii \"mytestfile\\000\"\n"
  "\n"
  ...
  "\n"
  "  lw	$10,%got(__afl_area_ptr)($28)\n"
  "  lw $10, 0($10)\n"
  "  beq $10, $0, __afl_setup\n" // 相等跳到 setup, 说明是刚启动
  "  nop\n"
  "\n"
  "__afl_store:\n"
  "\n"
  ...
  "\n" // 这里直接完成功能，先不考虑宏定义的情况
  "  lw $11, %got(__afl_prev_loc)($28)\n"
  "  lw $11, 0($11)\n"
  "  xor $11, $9, $11\n"
  "  addiu $2, $10, 0\n"
  "  addu $2, $2, $11\n" // 到这里$2是shared_mem[cur_location ^ prev_location]的地址
  "  lbu $3, 0($2)\n"
  "  addiu $3, $3 ,1\n"
  "  andi	$3,$3,0x00ff\n"
  "  sb $3, 0($2)\n"
  "  srl $9, $9, 1\n"
  "  lw $2, %got(__afl_prev_loc)($28)\n"
  "  sw $9, 0($2)\n"
  "__afl_return:\n"
  "\n"
  "  jr $31\n"
  ...
  "\n"
  "  lw $2, %got(__afl_setup_failure)($28)\n"
  "  lbu $2, 0($2)\n"
  "  bne $2, $0, __afl_return\n"
  "  nop\n"
  "\n"
  ...
  "  \n"
  "  lw $28,16($fp)\n"
  "  sw $2,20($fp)\n" // 把 getenv 的返回值保存在 20($fp)
  "  beq $2, $0, __afl_setup_abort\n"
  "  nop\n"
  "  lw	$4,20($fp)\n"
  "  lw $2,%call16(atoi)($28)\n"
  "  move $25,$2\n"
  ...
  "  \n"
  "  lw	$28,16($fp)\n"
  "  sw $2,20($fp)\n"  // 把 atoi 的返回值保存在 20($fp)
  "  move $6, $0\n"
  "  move $5, $0\n"
  "  move $4, $2\n"
  "  lw $2,%call16(shmat)($28)\n"
  "  move $25,$2\n"
  "  jalr $25\n"
  "  nop\n"
  "  \n"
  ...
  "  sw $2, 20($fp)\n"
  "  li $3, -1\n"
  "  beq $2, $3, __afl_setup_abort\n" // 上面的返回值在 $2
  "  nop\n"
  "\n"
  "  /* Store the address of the SHM region. */\n"
  "\n"
  "  lw $3, %got(__afl_area_ptr)($28)\n"
  "  sw $2, 0($3)\n"
  "  move $10, $2\n"
  "  lw $9, 24($fp)\n"
  "\n"
  ...
  "\n"
  "  li $6, 4\n"
  "  lw $5, %got(__afl_temp)($28)\n"
  "  li $4, " STRINGIFY((FORKSRV_FD + 1)) "  /* file desc */\n"
  ...
  "  sw $2, 20($fp)\n"
  "  li $3, 4\n"
  "  bne $2, $3, __afl_fork_resume\n"
  "  nop\n"
  /* 假设说 write 失败了，那有可能是没有用 forkserver 模式
     直接把这个进程当成运行 target 的进程就行了 */
  "\n"
  ...
  "  li $6, 4\n"
  "  lw $5, %got(__afl_temp)($28)\n"
  "  li $4, " STRINGIFY(FORKSRV_FD) "        /* file desc */\n"
  "  lw $2, %call16(read)($28)\n" // 阻塞, 等待 run_target
  "  move $25,$2\n"
  "  jalr $25\n"
  "  nop\n"
  ...
  "  nop\n"
  "\n"
  "  beq $2, $0, __afl_fork_resume\n" // pid == 0, 子进程
  "  nop\n"
  "\n" // 剩下的就是 pid >0, 父进程
  "  /* In parent process: write PID to pipe, then wait for child. */\n"
  "\n"
  "  lw $2, %got(__afl_fork_pid)($28)\n"
  "  lw $3, 20($fp)\n"
  "  sw $3, 0($2)\n"
  "  \n"
  "  li $6, 4\n"
  "  lw $5, %got(__afl_fork_pid)($28)\n"
  "  li $4, " STRINGIFY((FORKSRV_FD + 1)) "      /* file desc */\n"
  "  lw $2, %call16(write)($28)\n"
  "  move $25,$2\n"
  "  jalr $25\n"
  ...
  "  move $6, $0\n"
  "  lw $5, %got(__afl_temp)($28)\n"
  "  lw $4, %got(__afl_fork_pid)($28)\n"
  "  lw $4, 0($4)\n"
  "  lw $2,%call16(waitpid)($28)\n"
  "  move $25,$2\n"
  "  jalr $25\n"
  "  nop\n"
  "  \n"
  ...
  "  /* In child process: close fds, resume execution. */\n"
     /* 子进程不需要用到这些文件描述符，文件描述符是进程相关的 */
  "\n"
  "  li $4, " STRINGIFY(FORKSRV_FD) "\n"
  ...
  "  .set reorder\n"
  "  .end __afl_maybe_log\n"
  "  .size __afl_maybe_log, .-__afl_maybe_log\n"
  "/* --- END --- */\n"
  "\n";
